...
class Layer:
    def __init__(self, layer_shape, bsize, is_input=False, is_output=False,
                 activation=Activations.sigmoid):
        self.is_input = is_input
        self.is_output = is_output

        # Z is the matrix that holds output values
        self.Z = np.zeros((bsize, layer_shape[0]))
        ...
        if not is_input:
            self.S = np.zeros((bsize, layer_shape[0]))
            self.D = np.zeros((bsize, layer_shape[0]))

        if not is_output:
            self.W = np.random.normal(size=layer_shape, scale=1E-4)

        if not is_input and not is_output:
            self.Fp = np.zeros((layer_shape[0], bsize))
    ...
    def forward_propagate(self):
        ...
        if self.is_output:
            ...
            self.Fp = self.activation(self.Z, deriv=True).T
            return self.Z.dot(self.W)
